...
const PLATFORM_COLOR: Record<string, string> = {
  ...
  pinterest: '#E60023', x: '#000000', unknown: '#9CA3AF',
};
  ...
  template: `
    @if (mode() === 'thumbnail') {
      ...
      <span class="inline-flex shrink-0" [title]="label()">
        <ng-container *ngTemplateOutlet="iconSvg; context: { size: 20 }"></ng-container>
      </span>
    }

    <ng-template #iconSvg let-size="size">
      @switch (platform()) {
        @case ('youtube') {
          <svg [attr.viewBox]="'0 0 ' + size + ' ' + size" [attr.width]="size" [attr.height]="size" fill="none">
            <path [attr.d]="playPath(size)" fill="white"/>
          </svg>
        }
        @case ('tiktok') {
          <svg [attr.viewBox]="'0 0 20 20'" [attr.width]="size" [attr.height]="size" fill="none">
...
export class PlatformIconComponent {
  url = input<string>('');
  mode = input<'icon' | 'thumbnail'>('icon');
  platform = computed(() => detectPlatform(this.url()));
  label = computed(() => getPlatformName(this.platform()));
  color = computed(() => PLATFORM_COLOR[this.platform()] ?? '#9CA3AF');

  playPath(size: number): string {
    const m = size / 20;
    return `M${8*m} ${6.5*m}L${14*m} ${10*m} ${8*m} ${13.5*m}V${6.5*m}z`;
  }
}
